...
public:
	...
	std::cout << "PIConGPU sending signal\n";
        P_to_T = true;
	pthread_mutex_unlock(&mutex1);
	pthread_cond_signal(&condition1);*/

        const char * msg = "P to T";
	int bytes = nn_send (sockpush_p, msg, strlen (msg) + 1, 0);
	assert (bytes == strlen (msg) + 1);
        std::cerr << "[P] P to T" << std::endl;
	...
	std::cout << "PIConGPU received signal\n";
        T_to_P = false;
	...
	{
		char *buf = NULL;
		int bytes = nn_recv (sockpull_p, &buf, NN_MSG, 0);
		assert (bytes >= 0);
		std::cerr << "[P] : " << buf << std::endl;
		nn_freemsg (buf);
                break;
	} 
